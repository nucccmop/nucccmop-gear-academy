...
extern "C" fn state() {
    ...
    msg::reply(gmst, 0).expect("Failed to share state");
}
...
pub fn program_turn_gen(difficulty: DifficultyLevel, max_per_turn: u32, remain_num: u32) -> u32 {
    match difficulty {
        ...
        DifficultyLevel::Hard => {
            // Try to make the remaining amount N+1, so any number chosen by the user will make the remaining amount less than N
            let mut count = 0;
